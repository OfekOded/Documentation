...
md(f"""
<a id="step-40" name="step-40"></a>
## Step 40 — Realigning the TRUST defense to Adnane et al. (2013): implementing the two thirds of the paper that were missing
**Date:** 2026-08-18 – 2026-08-20

> **Chronology.** This work **overlaps** [Step 39](#step-39) rather than following it —
> TRUST 2026-08-18 → 08-20, Watchdog 2026-08-19 → 08-23. They are two independent audits
> of two defenses against their own source papers, run in parallel and written up in the
> order the write-ups arrived.

### Motivation — the same audit, applied to the fourth defense
[Step 19](#step-19) settled from the source header that TRUST implements **Adnane, Bidan &
de Sousa, *Computer Communications* 36 (2013)**. The publication argument of
[Step 39](#step-39) applies unchanged: a citation that the implementation cannot support
turns a defense-*detection* paper into a defense-*design* paper. So the implementation was
audited formula by formula against the source.

The answer was blunt. **Four of the paper's ten detection rules were implemented, Section 6
was not implemented at all, and Section 7 existed only as an idealised in-process bus** —
alerts were passed between defense objects inside the simulator rather than travelling over
the network as messages. Measured under the paper's own conditions the implementation
detected ≈ **21%** of attackers, raised **9–96%** false alerts depending on configuration,
and — with the counter-measure active — left PDR **below** the attack-only level.

> **What those figures do *not* say.** They characterise the **paper-faithful**
> configuration. The dataset harness enables only the forward monitor (Formula 10);
> consistency rules, provable identity and alert distribution are all **off** by default,
> so no dataset in this project was generated in the state described above. The paired-seed
> test at the end of this step confirms the default path is unchanged. `[VERIFIED]`

### What the paper specifies — three layers
| Layer | Role |
|---|---|
| **Section 5** | **Detection** — infer mistrust from contradictions between received messages |
| **Section 6** | **Prevention** — provable identity and neighbourhood proof, so that lying is not possible in the first place |
| **Section 7** | **Dissemination** — share the evidence with the rest of the network, plus the counter-measure that isolates the attacker |

The ten detection rules, paraphrased, and the verdict each yields:

| Formula | Contradiction | Mistrust |
|---|---|---|
| (6) | a node advertises TC selectors it never declared as symmetric neighbours in HELLO | exact |
| (7) | a node advertises `x` as its selector, but `x` never chose it as MPR | exact |
| (8) | two TC copies with the same originator and sequence number but different content, from two relays | **partial** |
| (9a) | a node selected as MPR never generates a TC at all | exact |
| (9b) | my own MPR's TC omits me from its selector list | exact |
| (10) | my own MPR forwards neither my DATA nor my TC | exact |
| (11)/(12) | nested symmetric-neighbour sets (`NSA ⊆ NSB`) where both are MPRs of the same node | **partial** |
| §5.1, prose | `y` advertises `z` in its TC, but `z` never declared `y` a symmetric neighbour | exact |

**The last rule is stated in prose, immediately after Formula (8), and is not numbered** —
which is presumably why it was never implemented. It is also the decisive one: it is the
**only rule that can fire at a node that is neither the attacker's neighbour nor named in
the attacker's messages**, and therefore the only one that lets the detection rate approach
the paper's reported 100%.

**Two caveats stated in the paper's text but absent from the implementation**, both of which
turned out to be load-bearing:

- **Validity time (§4).** HELLO and TC information is valid only for a stated period after
  reception; a contradiction must be evaluated against information that is still valid.
- **Persistence (§5.1).** The paper explicitly allows for a transient during discovery and
  requires mistrust only of a node whose contradictory messages *continue* to arrive after
  initialisation. The word doing the work is *continues*.

### What Table 3 measures — and what the paper never measures
**Table 3 is a raster image inside the PDF** (a JPEG XObject, 1417 × 1467), not selectable
text. It was extracted and its three panels read:

| Panel | Scenario | Runs | Result |
|---|---|---:|---|
| **3A** | 50 nodes, 1 attacker, 1 target | 26 | 100% in all 26; the exact component ≈ 77–100%, the remainder partial |
| **3B** | 50 nodes, 1 attacker, 2 targets | 20 | mostly 100%; lows of 62–88%; **zeros** in runs 10, 13, 15 |
| **3C** | 50 nodes, 2 attackers, 1 target | 20 | mostly 100%; lows of 67–87%; **zeros** in runs 6, 7, 13, 16 |

Three facts follow, and each constrains what may be claimed:

1. **The paper reports detection rate only.** Section 8 contains no PDR, no throughput and
   no delay. Every delivery-ratio number in this project's TRUST work is **our addition,
   with no reference value in the source.** Note the exact parallel with [Step 39](#step-39):
   neither Baiad paper measures packet delivery either. Both source papers stop at detection.
2. **The maximum is two attackers.** Not more.
3. **The paper reports no false alert at all** — a benchmark that cannot be waved away.

The only simulation parameters the paper gives are **50 nodes, random placement, no
mobility**. No area, no radio range, no traffic model, no duration; the simulator is
GlomoSim, not ns-3.

### Architecture after the realignment
Six sub-modules under `src/olsr/model/defense/`, wired and owned by `OlsrTrustDefense`:
`olsr-trust-config.h` (every parameter in one place), `olsr-consistency-rules.{{cc,h}}`
(§5 plus the extended prose rule), `olsr-forward-monitor.{{cc,h}}` (Formula 10),
`olsr-provable-identity.{{cc,h}}` (§6, Formulas 13 and 14), `olsr-trust-state.{{cc,h}}`
(`MN_x`, Formula 15, and the exact-vs-partial distinction), and
`olsr-alert-distributor.{{cc,h}}` (§7, proofs and rate limiting). `olsr-header` gains a
`PROOF_MESSAGE` type (5); `olsr-routing-protocol` gains the enforcement points, alert
broadcast and proof transmission.

**Detection is kept separate from response** — the design point [Step 19](#step-19) already
noted, now load-bearing for measurement. `GetBlacklist()` always reports what was detected;
`IsMalicious()`, which drives the counter-measure, returns true only when response is
enabled **and** an *exact* mistrust exists. That separation is what makes Table 3
reproducible at all: the paper measures §5 inference **without** counter-measures, and in
that mode the topology is left undisturbed.

Formula 15's enforcement points, in processing order: reject HELLO and TC originated or
relayed by a mistrusted node; exclude mistrusted neighbours from MPR set N and mistrusted
two-hop neighbours from N2; skip a mistrusted neighbour, its topology tuples and any
mistrusted gateway when computing the routing table; and drop a data packet whose next hop
is mistrusted.

### The five decisions that decided the numbers
Each is derived from the paper's wording, and each is recorded with its measured effect.

**1 — Persistence as a sliding window, not a counter.** `Persisted()` counts how often the
same contradiction recurs for the same (accused, witness) pair, and **resets the streak if
the contradiction stops recurring** within `PersistenceWindow`. A cumulative counter would
convict on two unrelated transients minutes apart, which is not what *continues* means.
The effect is the largest single result of this work: Formula (6) had been firing ≈ **142
times per run, almost all of it against honest nodes**, because TC and HELLO are emitted on
different timers (5 s against 2 s) so ordinary neighbourhood churn reads as a contradiction.
After the change: **zero.** This one change moved PDR recovery from **−102% to +53%**.
`[VERIFIED]`

**2 — Restricting which rules may broadcast an alert.** The paper requires proofs that
cannot be falsified. The broadcasting set is `{{6, 8, 12, 5.1e}}`. **Formula 7 was
deliberately removed from it:** its proof is the accuser's own private MPR set — a
self-declaration nobody else can verify. The original code had already kept Formula 9b
local for exactly this reason; Formula 7 has the same defect and had been left broadcasting
by oversight. Effect: `MN_x` fell from **13.72 to 5.62** and PDR recovery moved from
**−23% to +63%**. `[VERIFIED]`

**3 — The alert is a re-broadcast of the offending messages, not a new message type.** The
paper is explicit that no new message type is introduced, so the implementation stores the
raw `MessageHeader` of the messages that revealed the contradiction and re-floods them.
Three details were required:
- **HELLO is not forwarded under RFC 3626.** The restriction is lifted only for a HELLO
  marked as re-broadcast.
- **A re-broadcast HELLO is not a neighbourhood declaration.** Processed normally it would
  build link state toward the *relay* instead of the originator and contaminate the whole
  network. It is identified by a non-zero hop count and read as **evidence only**. The paper
  is aware of the phenomenon and states it does not harm routing.
- **A fresh sequence number per re-broadcast.** Otherwise OLSR's duplicate suppression
  deletes the alert precisely at the nodes that have not yet seen the evidence — which is
  its entire purpose.

**4 — Two checks at the receiver of an alert.** The paper requires the receiver to re-run
its own reasoning, so: reject an alert that our own **valid** local view actually
contradicts (absence of evidence is not a contradiction — only a real collision with a
still-valid observation disqualifies it), and reject an alert from an accuser already in
our own `MN_x`.

**5 — The proof is sent once and does not expire.** §6.2 sends the neighbourhood proof once,
on symmetric-link formation; the implementation sends only when the symmetric-neighbour set
**changes** — a burst at start-up, then silence in a stable network. Attaching a validity
time to an event-driven proof made every declaration unusable within about half a minute,
exactly in the stable networks where neighbourhood claims are most reliable. A proof is
**superseded** by the next one from the same node rather than ageing out.

**A protocol-level fix found on the way.** `SendQueuedMessages` packed messages into a
packet by **count** only, so a burst could build an OLSR packet exceeding the MTU. A
fragmented packet cannot be parsed from a single frame — and single-frame parsing is what
both the promiscuous monitors and the feature collector rely on. A byte budget now runs
alongside the count limit, per RFC 3626 §3.1.

### Measuring what the paper measures
**The dataset harness was not modified.** Measurement runs on
`scratch/olsr-trust-paper-replication.cc`, a **measurement-only copy** generated from it by
`mk_paper_replication_harness.py`; all four CSV schemas are byte-identical to the original's.
Five differences, each with its reason:

| ID | Change | Why |
|---|---|---|
| **PR-001** | accumulate detection across the whole measurement window (sampled every second) | the original samples **once**, at `winStart + 2 s` |
| **PR-002** | `--noGates` | the acceptance gates are not part of the paper's protocol, and the paper explicitly reports the isolated-target case that the gate would filter out |
| **PR-003** | `--responseEnabled=0` | Table 3 measures §5 alone. With the response active, a node that mistrusts the attacker stops forwarding its messages and thereby **destroys the evidence** the rest of the network needs |
| **PR-004** | report exact and partial separately | Table 3 plots them as two stacked series |
| **PR-005** | `--enableProvableIdentity` exposed | the original hard-codes §6 off |

> **PR-001 is the same defect as [Step 39](#step-39)'s `blacklist_max_size`.** Sampling at
> `winStart + 2 s` puts the reading **before** the window's data traffic and before a
> temporary mistrust has expired, so the harness reported **0%** false alerts where
> continuous sampling reports **11.7%**. Two defenses, two independent audits, the same
> trap: an oracle column read at an instant when the quantity it measures cannot yet be
> non-zero. The rule stated in [Step 39](#step-39) — locate a column's
> `Simulator::Schedule` before relying on it — now has two confirmed instances and should
> be treated as standing practice. `[VERIFIED]`

Each run measures four consecutive windows, 60 s of stabilisation then 40 s of measurement:

| Window | Attack | Defense | Role |
|---|---|---|---|
| `baseline` | off | off | the healthy network |
| `attack_only` | on | off | the damage the attack does |
| `defense_only` | off | on | **the cost of the defense in a clean network** |
| `defense_vs_attack` | on | on | the defense at work |

`defense_only` is the control that matters: any drop there is damage the defense causes with
no attacker present.

**Metrics.** *Detection rate* = `1 − avg_attacker_trust` in `defense_vs_attack` — the
fraction of (honest node, attacker) pairs whose `MN_x` contained the attacker at some point
in the window, which is exactly the paper's metric. *False positives* = the identical
computation in `defense_only`, where the same node exists and behaves honestly, so any
detection is a false accusation. *`MN_x`* = the largest suspect list held by an honest node.
*Recovery* = `(PDR_def+att − PDR_att) / (PDR_base − PDR_att)`.

**Topology:** 50 nodes, 750 × 1000 m, 190 m range, **static**, a single UDP flow
(node 1 → node 0), 18 packets of 512 B per window. Only the two parameters the paper states
— 50 nodes and no mobility — are matched; the rest are harness defaults, because the paper
gives no others. (That one-flow, 18-message load is the same traffic profile whose effect on
DCFM detectability [Step 38](#step-38) measured.)

### Results
**Table 3 replication — detection, counter-measures off.** 65 runs per row:

| Scenario | exact | partial | runs ≥ 90% | **false alerts** |
|---|---:|---:|---:|---:|
| **1 attacker** (against 3A) | **96.1%** | 1.1% | 86.2% | **0.0%** |
| **2 attackers** (against 3C) | **95.5%** | 0.9% | 83.1% | **0.0%** |

The paper reports 100% in most runs with a tail of 62–88% and a few zeros; this
distribution matches, at zero false accusations — the benchmark the paper sets.

**PDR — counter-measure active.** 1,413 runs in the recommended configuration (§5 + §7):
...
| attack_only | 69.58 |
| **defense_only** | **92.94** |
| **defense_vs_attack** | **82.74** |

**+13.16 PDR points absolute, 55.9% of what the attack took.** `defense_only` at 92.94
against a baseline of 93.13 says the defense is effectively free in a clean network.

**Before and after:**

| | before | after |
|---|---:|---:|
| detection rules implemented | 4 of 10 | **10 of 10** |
| detection (Table 3 conditions) | 21% | **96.1%** |
| false alerts | 9–96% | **0.0%** |
| PDR recovery | −102% | **+55.9%** |
| PDR of a clean network | 73 / 35 | **92.94 ≈ baseline** |

**Should Section 6 be switched on?** Both configurations are faithful to the paper. With
Formula 14 fixed and actually working:

| Configuration | runs | detection | false | recovery |
|---|---:|---:|---:|---:|
| **§5 + §7 (recommended)** | 1,413 | **91.1%** | 0.55% | **+55.9%** |
| §5 + §6 + §7 | 613 | 65.2% | 0.49% | +39.8% |

Under Table 3 conditions (no counter-measure) §6 changes nothing — 96.1% either way. With
the counter-measure it is **harmful on every axis**. The reason is mechanical: the extended
§5.1 rule already catches the same lie from the HELLO, while §6 adds proof traffic and
isolates the attacker **earlier**, which stops its TC flooding and prevents the rest of the
network from ever seeing the evidence. It is a *preventive* mechanism, and here prevention
costs more than it returns. **Operational recommendation:
`--enableProvableIdentity=0`** — §6 is implemented, faithful and tested, but better left off
against this attack model.

**A structural tension worth carrying forward:** the faster the response, the *lower* the
measured detection rate, because a node that mistrusts the attacker stops relaying its
control messages and distant nodes never see the evidence. Actual protection improves while
the measured number falls. This is very likely why the paper measures Table 3 on §5 alone
and presents the counter-measures separately.

### The QA round — four critical defects, none visible in the aggregate numbers
Six defects were found by a targeted re-read of the code. **The aggregate results looked
good throughout, because the extended rule and the alert distribution were covering for
them.**

| # | Defect | Severity |
|---|---|---|
| QA-1 | **`Digest` used XOR, which cancels.** Adding any address **twice** leaves the digest unchanged, the signature still verifies, and `ProveNeighborhood` reports it as declared — a complete forgery of the declaration Formula 14 rests on. Replaced with a position-dependent fold over a canonical form, and the canonical form is enforced on receipt | **critical** |
| QA-2 | `Proof::Deserialize` used `NS_ASSERT` on a short message, so a truncated PROOF aborts the whole simulation. It now returns an empty proof that fails verification and is rejected | **critical** |
| QA-3 | **The prime table contained primes with `p ≡ 1 (mod 3)`**, for which `gcd(3, p−1) ≠ 1` and no modular inverse of `e = 3` exists. About half the nodes got a broken key: their signatures never verified, their proofs were always rejected, and Formula 14 never worked for them. Replaced with 120 primes `p ≡ 2 (mod 3)`, with an assertion that the inverse is real | **critical** |
| QA-4 | `m_declaredEverSent` was not reset in `DoDispose` while `m_identity` was destroyed. After the first cold start **no node ever broadcast a proof again**, every store stayed empty, and **Formula 14 was silently disabled in exactly the measured window** | **critical** |
| QA-5 | `m_tcCopies` (the Formula-8 cache) was keyed on a non-recurring sequence and grew without bound. Eviction by `topologyValidity` added | medium |
| QA-6 | `m_rawTc` was written on every TC and never read. Removed | minor |

QA-4 belongs to the same family as the sampling defects above: a mechanism that reports
success while being inert. A ten-test unit suite for the signature scheme
(`qa_digest_test.cc`) now **reproduces the QA-1 forgery and proves it is blocked**, alongside
canonical-form rejection, cross-node signing, distinct moduli for all 50 nodes and
sign/verify round-trips. All ten pass.

### Validation
| Check | Result |
|---|---|
| clean build from scratch | **passes, no warnings** |
| `routing-olsr`, `routing-olsr-header` | **PASS** |
| `routing-olsr-regression` | **crashes — pre-existing.** Proved by building a clean tree at the restore-point tag and running the same suite: identical crash. Cause is a `MacTxDrop` Wifi trace connected in a point-to-point topology; the code is byte-identical to the original tree |
| crash sweep, 20 runs | **20/20 clean** — 1/2/4 attackers, mobility, 12/50/80 nodes, high range, `spoofCount=20`, permanent mistrust, shuffled window order, gates on and off, and no attacker at all |
| **soak: 2,026 consecutive runs** | **0 crashes, 0 errors**, memory flat (1,002 MB at run 87 and 1,002 MB at run 1,137) |
| rule coverage across seeds | 5.1e, 7, 14, 12 and 10 fire; 6, 8, 9a and 9b do not — correct for this attack model: Formula 8 needs an accomplice that does not exist here, and the MPRs do generate TCs |

Zero failures in 2,026 runs gives a 95% upper bound of **0.148%**, i.e. up to ~15 failures in
10,000 runs. That is a statistical inference, not a guarantee, but it is measured on the
exact code a campaign would run. Runs ending `rc=2` are legitimate seed rejections by the
harness, not failures.

### Impact on the existing dataset: none
A **paired run on identical seeds**, original tree against the realigned code, using the
**unmodified** dataset harness in its default configuration:

| Window | PDR before | PDR after | control bytes | Δ |
|---|---:|---:|---:|---:|
| baseline | 100.00 | 100.00 | 277141 vs 277141 | **0.00** |
| attack_only | 69.05 | 69.05 | 277159 vs 277159 | **0.00** |
| defense_only | 100.00 | 100.00 | 277348 vs 277348 | **0.00** |
| defense_vs_attack | 70.11 | 70.11 | 276802 vs 276802 | **0.00** |

**42 of 42 shared seeds identical.** The reason is structural: the harness default enables
only the forward monitor, and every component this work changed is off in it. **The existing
`trust_static/` and `trust_dynamic/` datasets, and every TRUST result in
[Part III](#step-20) and [Part V](#step-27), stand unchanged.** [VERIFIED — paired seeds]

> ⚠️ **A separate warning about `overhead_ratio`.** `simulations/trust_static/` was generated
> with `harness_version=2.3.0` (three data flows); the current harness is `2.4.0` (one flow,
> TRF-005). The metric is control bytes over `rxBytes`, so a third of the data bytes triples
> it (9.98 against 28.37). This **predates** the present work, but the column must not be
> mixed across datasets from the two harness versions.

### Declared limitations
- **Key length.** The signatures are genuinely asymmetric — signing needs the private key,
  verification uses only the public key carried in the message, and no node can sign for
  another, all checked by the unit suite — but the modulus is a product of two five-digit
  primes. These are **simulation-grade keys**. The paper specifies no key length at all; it
  assumes SOLSR provides signatures. What is measured here is protocol behaviour, not
  cryptographic strength, and key length affects no measurement except message size. Padding
  the signature and key fields to RSA-2048 would take a PROOF message from ≈ 66 to ≈ 565
  bytes; the **recommended configuration sends no PROOF messages at all**, so it would be
  unaffected.
- **The attack model is not the paper's scenario 1**, and was not changed to be. This
  project's attacker advertises **real distant nodes** rather than a fictitious node `X`,
  and adds a **black hole** on the data plane, which the paper treats as a separate attack;
  its scenario 1 is passive. This is a difference in the scenario under test, **not a gap in
  the defense** — but the attack must be described as it is and not implied to be the
  paper's.
- **The partial series is smaller than the paper's.** Formulas 8 and 12 are implemented and
  do fire (12 about 14 times per run), but under this attack model they rarely incriminate
  the **attacker itself** — Formula 8 needs an accomplice that does not exist here. Measured
  partial is 0.9–1.1%, against a larger partial component in some of the paper's runs.

### Consequence for the ML campaign
1. **The dataset is unaffected.** [VERIFIED] above — nothing needs regenerating for the
   results already reported.
2. **But the paper-faithful configuration is a behaviourally different defense.** §7 alert
   distribution *re-floods control messages with fresh sequence numbers* — additional
   control-plane traffic, which is exactly the class of signature [Part III](#step-24) found
   detectable. TRUST2 has been the project's *gentle, near-invisible* defense: no Core-95
   feature exceeds a univariate AUC of 0.64 against it ([Step 19](#step-19),
   [Step 31](#step-31)), and it sits near chance in cross-defense transfer
   ([Step 36](#step-36)). A TRUST running §5 + §7 should be **substantially louder** —
   `[HYPOTHESIS]`, and a direct test of the [Step 24](#step-24) tradeoff thesis on a defense
   whose efficacy has now been raised from harmful to +55.9% recovery. Regenerating TRUST in
   that configuration is therefore **a new condition, not a refresh**, and must be labelled
   as one; the Trust 1.0 / 2.0 calibration pair of [Step 24](#step-24) is the natural place
   for it as a third point.
3. **Version provenance.** This work is based on **ns-3.47** in a `ns-3-dev` tree, while the
   dataset was generated on **ns-3.45** ([Step 14](#step-14)). Any campaign that mixes the
   two must resolve that first. `[VERIFIED — restore-point tag is a merge of ns-3.47]`

### Sources
- Defense, as tracked in the public repo (**pre-realignment**): {ref(D_TRUST)},
  {ref(D_TRUST_H, "olsr-trust-defense.h")}
- Dataset harness, **unmodified by this work**:
  {ref("files for all defenses/Trust/olsr-trust-eval-mitigation.cc", "olsr-trust-eval-mitigation.cc")}
- **New/changed on branch `trust-defense` — not in `manet-olsr-project` master:** six
  sub-modules under `src/olsr/model/defense/` (`olsr-trust-config.h`,
  `olsr-consistency-rules`, `olsr-forward-monitor`, `olsr-provable-identity`,
  `olsr-trust-state`, `olsr-alert-distributor`); `PROOF_MESSAGE` (type 5) in `olsr-header`;
  the Formula-15 enforcement points, `BroadcastTrustAlert` and proof transmission in
  `olsr-routing-protocol`
- Measurement copy: `scratch/olsr-trust-paper-replication.cc`, generated by
  `mk_paper_replication_harness.py`
- Outside both repositories: `TRUST_OLSR_DOCUMENTATION.md` (the full write-up this step
  condenses), `trust_paper_conformance.md`, `trust_defense_paper_fixes.patch`,
  `qa_digest_test.cc`
- Git: branch `trust-defense`, commit `9f18f0304`, tag
  `trust-defense-paper-complete-2026-08-20`, restore point
  `trust-defense-verified-2026-08-18` (`41f9a82e0`); **17 files, +1,440 / −72 lines**
- Paper: Adnane, Bidan & de Sousa Júnior, *Computer Communications* **36** (2013) 1159–1171
- Antecedents: [Step 19](#step-19) (TRUST added, and the detection/response separation);
  [Step 24](#step-24) (the Trust 1.0 / 2.0 calibration pair); [Step 31](#step-31) (no Core-95
  feature above 0.64); [Step 39](#step-39) (the parallel Watchdog audit, and the sampling rule
  this step confirms)

> **A note on paths.** The realignment was done in a `ns-3-dev` working tree whose layout
> predates the 2026-08-04 reorganisation ([File Index](#file-index)) — TRUST still sits in
> `src/olsr/model/` and its harnesses in `scratch/`. Merging this branch into
> `manet-olsr-project` master will have to move the six new `defense/` files into
...
md(f"""
<a id="step-43" name="step-43"></a>
## Step 43 — FPNT and TRUST on the 17-observable schema: a byte-padding artifact, and a defense that never wakes up
**Date:** 2026-08-30 → 2026-09-01

### Motivation — the other two defenses, and the fourth corner of the tradeoff

[Step 42](#step-42) ran the first two campaigns on the single-listener schema of
[Step 41](#step-41), but it ran the two defenses that were already the study's best-understood
pair. FPNT and TRUST had never been measured through a uniformly one-radio observation model at
all. They are also the two the tradeoff thesis most needs: FPNT has been the study's *most*
detectable defense since [Step 21](#step-21), and TRUST2 its quietest since [Step 31](#step-31),
so the two of them bracket the range the thesis is a claim about.

This step generates four batches — FPNT and TRUST, static and mobile, ~2,000 accepted runs each —
measures detectability and efficacy on both, and then runs one ablation that changes what the
FPNT result means.

### A. Four batches, 8,034 runs, 32,136 windows

Generated 2026-08-30 on two branches of the NS-3 repository, each carrying the schema-v6
collector: `fpnt-defense` at `cae7c4c69` and `trust-defense` at `f9a2e9702`. Both build clean
against ns-3.47 and pass `--self-test`.

| Batch | Branch | Runs | Windows | Yield | Wall | Seed base |
|---|---|---:|---:|---:|---:|---|
| FPNT static | `fpnt-defense` | 2,005 | 8,020 | 0.491 | 2.29 h | 1 |
| FPNT mobile | `fpnt-defense` | 2,010 | 8,040 | 0.344 | 1.98 h | 2.0e6 |
| TRUST static | `trust-defense` | 2,010 | 8,040 | 0.489 | 1.35 h | 4.0e6 |
| TRUST mobile | `trust-defense` | 2,009 | 8,036 | 0.343 | 1.67 h | 6.0e6 |
| **Total** | | **8,034** | **32,136** | — | **7.29 h** | disjoint ranges |

16,433 attempts, 8,034 accepted, **zero simulator errors**; `windows = runs × 4` exactly in every
batch, class balance `pos_rate = 0.5000` by construction (two ON and two OFF windows per run),
`random_window_order = 0` throughout, and **zero `NaN` rows**, so the `dropna` guard added in
[Step 42](#step-42) dropped nothing here either. [VERIFIED]

**The acceptance criterion is now identified, closing an open item from [Step 42](#step-42).**
That step measured the runner's rejection rate at ~51% static and ~66% mobile, showed it identical
across defenses, and left *what the criterion is* explicitly unaudited. It is a **connectivity
assertion**: at `t = 60 s`, in the neutral state, node 0 must reach at least 49 distinct
destinations. **All 8,399 rejections across these four batches were `assert_connectivity`** —
not a single one from any other cause. The yields here (0.491 / 0.344 / 0.489 / 0.343) match
[Step 42](#step-42)'s figures to the third decimal, so the same gate produced both campaigns.
[VERIFIED]

This confirms the `[HYPOTHESIS]` [Step 42](#step-42) attached to the finding: the datasets **are**
conditioned on something PDR-adjacent. Every accepted run is one in which the network was
well-connected *before* the attack started. That is a defensible experimental control — it removes
runs where a low PDR would say nothing about the attack — but it is a conditioning event and
belongs in any statement of the population these results describe. [VERIFIED]

### B. Two configuration differences from [Step 42](#step-42), and how far they travel

The two campaigns are **not** the same experiment run four times. Read back from `runs.csv`,
constant across all 8,034 rows:

| | [Step 42](#step-42) (DCFM, Watchdog) | This step (FPNT, TRUST) |
|---|---|---|
| attackers | 2 | **1** (node 2) |
| listener | node 2 (= first attacker) | node 2 (= the attacker) |
| feature-set flag | `--feature-set all` → **17 base → 87** | default preset → **16 base → 82** |
| run length | 400 s canonical (WIN-003) | **460 s** (60 s neutral prologue + 4 × 100) |
| nodes / field / range / flows | 50 / 750×1000 m / 190 m / 1 | identical |

Two of these matter and one is unresolved.

**The feature-set flag drops `RoutingOverheadBytesRatio`.** These runs used the default preset,
whose intersection with the 17 names is only 16 — exactly the omission [Step 42](#step-42) warned
about, and exactly the column that topped its Watchdog/static importance list. So the base width
here is 16, expanding to 82 rather than 87. Whether that column would have changed either result
is untested; §D shows it is byte-derived and would have been dropped by the ablation anyway, but
that argument does not cover the un-ablated numbers. [VERIFIED — the counts; the consequence is
untested]

**The attacker count did not move the attack-off rows much, but it did move `attack_only`.**
Across the two campaigns `baseline` agrees to 0.01 pp in static (100.00 / 99.99 / 100.00) and
0.74 pp in mobile (76.71–77.45); `attack_only` static spreads more widely, 71.67 (TRUST) against
73.81–73.86 for the other three. The 2.1 pp outlier is a one-attacker batch against two-attacker
ones and cannot be attributed to the defense, which is off in that scenario. Cross-campaign
efficacy comparisons in §F are therefore reported, but should be read at a resolution no finer
than ~2 pp — coarser than the 5.56 pp PDR quantum in any case. [VERIFIED]

**The 460 s run length contradicts [Step 41](#step-41) and is unresolved.** WIN-003 made the
neutral prologue conditional and put canonical order at **400 s**; these batches ran 460 s with an
unconditional 60 s prologue, on branches whose collector is otherwise schema v6 at
`HARNESS_VERSION 3.0.0` / `HEADER_VERSION 8`. The most economical reading is that the two defense
branches carry the new collector on a pre-WIN-003 harness base, so the version strings agree while
the timing does not. Nothing in these results depends on it — the prologue precedes every
measurement window and the gate fires neutral in both modes — but the version string is then not a
sufficient description of a tree, and the four batches of this step and the four of
[Step 42](#step-42) do not share a run length. [HYPOTHESIS — the constants are quoted from the
harness source; the branch bases were not diffed]

### C. Detectability — one defense apparently perfect, one apparently invisible

{refml(ML_PIPE)}, task `presence_active` (label `defense_enabled`, independent of attacker state),
2×5 `StratifiedGroupKFold` grouped by `run_id`, in-fold isotonic calibration, Nadeau-Bengio
corrected CIs, 13 models including the `Dummy` floor.

| Condition | Best model | ROC-AUC | 95% CI | MCC | TPR@1% FPR | Bal. acc |
|---|---|---:|---|---:|---:|---:|
| FPNT static | Ridge | **0.9875** | [.9853, .9898] | 0.904 | 0.880 | 0.952 |
| FPNT mobile | Ridge | **0.9924** | [.9902, .9947] | 0.934 | 0.932 | 0.967 |
| TRUST static | ExtraTrees | **0.5585** | [.5491, .5679] | 0.080 | 0.019 | 0.539 |
| TRUST mobile | LogisticRegression | **0.6059** | [.5945, .6173] | 0.161 | 0.031 | 0.579 |
| `Dummy` (all) | — | 0.5000 | [.5000, .5000] | 0.000 | 0.010 | 0.500 |

On FPNT the **linear models win** — Ridge and LogisticRegression at 0.987–0.992, ahead of
RandomForest and ExtraTrees at 0.974–0.979. A sharp near-linear separation is the signature of a
threshold, not of a behavioural signature spread across features; it is the same shape
[Step 21](#step-21) described as a disguised IF rule. In the weak conditions the ordering inverts
toward ExtraTrees and Stacking, mirroring the model-rank inversion [Step 42](#step-42) found on
Watchdog. [VERIFIED]

**TRUST's signal is real and useless, and both halves need saying.** The CI [.5491, .5679]
excludes the 0.5000 chance floor, so 8,040 windows do detect *something*. But MCC is 0.080 and
**TPR at a 1% false-alarm rate is 1.9%** — an adversary acting on this detector is wrong
essentially always. ROC-AUC is the most forgiving of the three metrics here; reporting it alone
would overstate TRUST by a wide margin. Note also that the `Dummy` F1 is 0.6667 in every
condition, above TRUST's own 0.546 — F1 must not be read as a performance figure on this task.
[VERIFIED]

### D. The FPNT signal is byte padding — the ablation

Per-scenario means on FPNT/static make the mechanism visible without any model:

| Scenario | `AvgTxPacketSize` | `Throughput` | `TcMessageRate` | `AvgAdvertisedLinks` |
|---|---:|---:|---:|---:|
| `baseline` | 207.01 | 13,563 | 5.856 | 4.294 |
| `attack_only` | 206.89 | 12,393 | 5.861 | 4.450 |
| `defense_only` | **252.22** | 16,731 | 5.844 | 4.296 |
| `defense_vs_attack` | **245.62** | 15,821 | 5.845 | 4.451 |

Mean sniffed frame size rises ~45 B the moment the defense is enabled, **identically with and
without an attacker**, while `TcMessageRate` does not move (5.856 → 5.844) and
`AverageAdvertisedLinksPerTCMessage` does not move (4.294 → 4.296). Cohen's *d* on
`AvgTxPacketSize` is **+1.402** static and **+1.352** mobile; the five largest effect sizes in
both mobilities are *exactly* the five byte-derived features, and the sixth-largest is 0.282.
The arithmetic closes: frames +6%, frame size +20%, 1.06 × 1.20 ≈ 1.27 against an observed +25%
in `Throughput` — the byte increase is frame count × frame size, with **no new control traffic**.
[VERIFIED]

This is the guide's own framing applied exactly: a behavioural feature changes because the network
behaves differently; an artifact changes because the defense **writes** it differently. A quantity
that responds to the defense and is indifferent to the adversary is a constant overhead, not a
response.

**The existing mitigation does not work.** `RoutingOverheadBytesRatio` was excluded from `METRICS`
a priori to remove precisely this artifact, but the ratio does not move at all (0.8796 → 0.8798,
*d* = +0.001): with ~94% of heard IPv4 frames being OLSR control traffic, numerator and denominator
scale together. The padding re-enters through the **absolute** byte features instead. [VERIFIED]

**The ablation.** Dropping all five byte-derived features — `AvgTxPacketSize`, `AvgFlowThroughput`,
`AvgTxBytesPerFlow`, `Throughput`, `FlowThroughputStd` — takes the base set 16 → 11 and the
expanded matrix 82 → 57 columns, confirming that the composite gate prevents a dropped base
feature from re-entering through a derived column:

| Condition | ROC-AUC before | after | MCC before → after | TPR@1% before → after |
|---|---:|---:|---|---|
| FPNT static | 0.9875 | **0.6027** | 0.904 → 0.139 | 0.880 → 0.090 |
| FPNT mobile | 0.9924 | **0.6194** | 0.934 → 0.173 | 0.932 → 0.074 |

**FPNT's near-perfect detectability is an implementation artifact of message padding, not a
property of the method.** [VERIFIED]

**This replicates [Step 21](#step-21) on a different schema and a different vantage point.**
Campaign 1 found FPNT at ROC-AUC ≈ 1.00 on 95 network-wide features, attributed it to
`TcMessageSizeMean`, and collapsed it to ≈ 60% by removing seven TC-byte-size features. Campaign 3
finds 0.99 on 16 single-listener observables, attributes it to five byte-derived features, and
collapses it to 0.60–0.62. Two independent feature universes and two different observation models
land on the same number. The finding is not an artifact of either schema. [VERIFIED]

> **This settles Open Question 1**, which [Step 34](#step-34) had recorded as *addressed* in the
> other direction. That step's static↔mobile transfer test (ROC-AUC 1.000 both ways) was read as
> showing the TC-size signal to be a legitimate, generalising finding. The transfer test was sound
> but does not bear on the question: a **constant** padding overhead generalises across mobility
> precisely *because* it is constant. Generalisation across regimes is not a test of
> inherent-versus-artifact; indifference to the adversary is, and that test says artifact.

### E. TRUST is dormant in static topologies

`AverageAdvertisedLinksPerTCMessage`, per scenario:

| Condition | `baseline` | `attack_only` | `defense_only` | `defense_vs_attack` | defense effect |
|---|---:|---:|---:|---:|---:|
| TRUST static | 4.3022 | 4.4516 | 4.3007 | 4.4491 | **−0.03%** |
| TRUST mobile | 4.2059 | 4.3287 | 4.1005 | 4.2588 | **−2.5%** |

In static topologies TRUST's `defense_only` windows are indistinguishable from `baseline`
(4.3007 vs 4.3022) and its paired PDR cost is −0.02 pp at *t* = −1.10 — nothing, on either the
control plane or the data plane. The configuration explains it: `min_forward_failures = 3` with
`forward_timeout_s = 3`, so nothing is mistrusted unless forwarding actually **fails**. In a
static, well-connected network it does not. **The defense never acts, so there is nothing to
detect.** Under mobility the mechanism is provoked, prunes 2.5% of advertised links, and the AUC
rises 0.5585 → 0.6059. [VERIFIED]

**Mobility exposes reactive defenses instead of masking them.** The project's earlier reading —
that mobility adds noise and masks behavioural signals — holds for *passive* signatures, where a
defense writes the same thing regardless of topology. It inverts for *reactive* ones, which need
a failure to trigger at all. TRUST is the clean case: it is more detectable under mobility because
mobility is what makes it run. [VERIFIED — the ordering and the mechanism configuration; the
generalisation to other reactive defenses is untested]

Two cautions on that table. `AverageAdvertisedLinksPerTCMessage` tracks the **attacker** (4.30 →
4.45) far more strongly than the defense — on this schema it is an attack detector, not a defense
detector, and must not be quoted as the latter. And TRUST ran here in the harness default —
`enable_consistency_rules = 0`, `enable_alert_distribution = 0`, forward monitor only — so this is
the **control point** for Open Question 15, not an answer to it. The §5 + §7 configuration
[Step 40](#step-40) built has still never been generated. [VERIFIED]

### F. Efficacy, and the four-defense picture

Mean `pdr_percent` from `windows_oracle.csv` — ground truth, never a model input:

| Condition | `baseline` | `attack_only` | `defense_only` | `defense_vs_attack` |
|---|---:|---:|---:|---:|
| FPNT static | 100.00 | 73.86 | 99.12 | 90.55 |
| FPNT mobile | 76.71 | 58.43 | 81.03 | 71.29 |
| TRUST static | 99.99 | 71.67 | 99.97 | 74.45 |
| TRUST mobile | 76.85 | 58.51 | 74.78 | 60.70 |

**The paired, damage-restricted measure is the honest one.** Restricted to runs where the attack
cost more than 0.5 pp of PDR, compared within run:

| Condition | damaged runs | under attack | with defense | baseline | **recovered** | restored to baseline |
|---|---:|---:|---:|---:|---:|---:|
| FPNT static | 526 (26.2%) | 0.36% | 74.47% | 100.00% | **74.4%** | 6.3% |
| FPNT mobile | 1,136 (56.5%) | 35.69% | 65.41% | 77.69% | **70.7%** | 42.1% |
| TRUST static | 575 (28.6%) | 0.96% | 12.54% | 99.97% | **11.7%** | 2.8% |
| TRUST mobile | 1,149 (57.2%) | 36.36% | 49.16% | 78.03% | **30.7%** | 29.2% |

Averaging over unaffected runs dilutes this by roughly 4× — in 71–74% of static runs the attacker
does not sit on the single UDP path and the attack does nothing at all, which is why every static
median is 100% while the means move. PDR is also quantised: one flow of 18 expected packets per
window gives a quantum of 1/18 = **5.56 pp**, so any difference smaller than that is a shift in the
distribution across runs, not a change within a run.

Cost with no attacker present, paired `defense_only − baseline`: FPNT static **−0.88 pp**
(*t* = −10.46, small but real); FPNT mobile **+4.33 pp** (*t* = +7.01 — it *improves* delivery
even unattacked); TRUST static **−0.02 pp** (*t* = −1.10, no effect); TRUST mobile **−2.07 pp**
(*t* = −3.15, false-positive pruning of healthy links). [VERIFIED]

**The four defenses on one axis.** Recomputing this step's recovery rate with
[Step 42](#step-42)'s formula — `(defense_vs_attack − attack_only) / (baseline − attack_only)` on
all-run means, not the damage-restricted subset — puts all four on a comparable scale for the
first time:

| Defense | recovery static | recovery mobile | ROC-AUC static | ROC-AUC mobile |
|---|---:|---:|---:|---:|
| FPNT | **63.8%** | **70.3%** | 0.60 † | 0.62 † |
| Watchdog | 50.5% | 26.9% | 0.59 | 0.60 |
| DCFM | 30.0% | −2.6% | **0.96** | **0.97** |
| TRUST | 9.8% | 11.9% | 0.56 | 0.61 |

† FPNT after the §D ablation; **0.99 / 0.99 before it**. Watchdog and DCFM from
[Step 42](#step-42), under the configuration differences of §B.

**The tradeoff thesis survives, but the mechanism it names should be restated.** The one defense a
passive listener can actually detect (DCFM, 0.96–0.97) is also the *least* effective of the four
and the only one with a negative recovery rate. The most effective (FPNT, 63.8–70.3%) is, once its
padding is removed, indistinguishable in detectability from the two quiet defenses. So the data do
**not** support a smooth monotone tradeoff between efficacy and detectability — TRUST is both the
least effective and among the least detectable, which a monotone reading forbids. What the four
points do support is that **detectability tracks how intrusively a defense writes to the control
plane, and efficacy is a separate axis.** DCFM injects fictitious nodes and is loud; FPNT padded
bytes and was loud for that reason alone; Watchdog and TRUST write nothing and are quiet whether
they work (Watchdog) or barely act (TRUST). [HYPOTHESIS — the four measurements are verified; the
reading across them is an interpretation, and rests on two campaigns that differ in the ways §B
records]

### G. What this does not establish

- **The residual FPNT signal is unexplained.** After ablation, 0.60–0.62 rests on
  `NormalizedRoutingLoad` (*d* = −0.18 / −0.28) and `DataPacketRate` (*d* = +0.16 / +0.19) — more
  frames heard, a smaller share of them OLSR — while `TcMessageRate` stays flat, so these are not
  new control messages. The plausible reading is that fatter frames fail more often and provoke
  retransmissions, a second-order echo of the same padding. The decisive test is one more ablation
  dropping those two as well. **NOT RUN.** [HYPOTHESIS]
- **TRUST/static's signal has no identified carrier.** Every univariate |*d*| < 0.05 and the top
  permutation importance is 0.017, yet the CI excludes chance. It appears to be a multivariate
  aggregation of many *d* ≈ 0.01–0.05 effects that 8,040 windows suffice to detect. A grouped
  label-permutation test of the kind [Step 42](#step-42) ran on Watchdog would characterise the
  null directly. **NOT RUN.** [HYPOTHESIS]
- **One attacker, one flow, one topology family.** 1 attacker at 5 spoofed links, 1 UDP flow,
  50 nodes at 190 m in a fixed 750 × 1000 m field. Nothing here speaks to multiple attackers,
  denser traffic, or other geometries — and [Step 38](#step-38) already showed that traffic load
  alone can reverse a static↔mobile ordering.
- **The vantage is adversarial, not neutral.** As [Step 41](#step-41) flagged, `listener_node`
  equals the attacker id in all 8,034 runs. These results answer *can the adversary tell it is
  being defended against*, not *can any node tell a defense is running*.
- **Window order was canonical in every batch.** `random_window_order = 0` throughout, so slot
  position and scenario are perfectly confounded. The harness supports `--randomWindowOrder`;
  it was not exercised.
- **No cross-defense transfer.** Every model here is trained and tested on one defense; whether a
  detector trained on FPNT recognises TRUST is untested on this schema.
- **The datasets are on one machine and not under version control.** The master copy sits outside
  the NS-3 repository and the working copy is git-ignored; the machine hard-powered-off twice on
  2026-08-31 with no BugCheck, dump or WHEA event, which points at a power or thermal cut rather
  than software. The branch commits are recoverable; 7.3 hours of simulation output is not.

### Two operational notes from the run

The 2026-08-31 model runs had to be repeated at `MAX_JOBS=2` with BLAS threads capped. The likely
load source is visible in the artifacts: fitted against a near-noise label the forests grow to
maximum depth — `trust17_static`'s ExtraTrees is **97 MB** against 8 KB for the Ridge models.
And raising the simulation workers 6 → 12 mid-run bought only a measured **1.22×** (0.556 → 0.676
attempts/s): the i9-12900H has 6 P-cores, so workers 7–12 land on E-cores and SMT siblings.

After a 2026-09-01 pipeline update (a `dump_model_params` provenance dump) all four main conditions
were re-run: ROC-AUC identical to six decimals, largest deviation across 52 model × condition cells
0.0053, attributable to XGBoost thread non-determinism. [VERIFIED]

### Sources
- Collector: {ref(FEATURES)} (schema v6, 17 columns), on branches `fpnt-defense` (`cae7c4c69`)
  and `trust-defense` (`f9a2e9702`)
- Defenses: {ref(D_FPNT)}, {ref(D_TRUST)} — parameters recorded per batch in `defense_params.txt`
  (provenance only, never a model input)
- Pipeline: {refml(ML_PIPE)} — `--defense fpnt17|trust17`, `--mobility static|mobile`, and
  `--drop-features AvgTxPacketSize AvgFlowThroughput AvgTxBytesPerFlow Throughput FlowThroughputStd`
  for the §D ablation
- Datasets (machine-local, **not** in either repository): master
  `~/olsr-batch/out/{{fpnt,trust}}_{{static,mobile}}/`; md5-identical working copy
  {refml(DML + "/dataset_final", "dataset_final/{fpnt_17,trust_17}/{static,mobile}_canonical/")}
- Outputs: {refml(DML + "/results/30_schema33/paper_v4", "paper_v4/")} —
  `fpnt17_{{static,mobile}}/`, `trust17_{{static,mobile}}/`, `fpnt17_{{static,mobile}}_nobytes/`,
  each with `summary.csv`, `folds.csv`, `importance.csv`, `run_config.json`, `model_params.json`,
  `summary.tex`, `final_model.pkl`, `figures/`
- Antecedents: [Step 41](#step-41) (the 17-observable schema and the adversarial vantage);
  [Step 42](#step-42) (the sibling campaigns this one is compared against);
  [Step 21](#step-21) (the FPNT byte artifact, first found on the 95-feature schema);
  [Step 24](#step-24) (the tradeoff thesis); [Step 40](#step-40) (the TRUST realignment whose new
...
**Merging the two produced a third instance of the same failure mode, and this one is worth a
rule.** The two `dcfm17` entries sit on different lines, so `git merge` resolved them **cleanly
and silently** — no conflict, both kept. A Python dict literal with a repeated key is not an
error: it keeps the **last** one. So `dcfm17` resolved to the collaborator's form while the
entry added here sat six lines above it, syntactically valid, permanently dead, and looking
authoritative to anyone reading the file. The duplicate was removed and the collaborator's form
kept, the staged data moved to `dataset_final/dcfm/Pilot2k/{{static,mobile}}` to match it. The
rule: **after merging a branch that touched `DATASET_LAYOUT` or `METRICS`, check for repeated
keys — a clean merge is not evidence of a correct one.** `python -c "import
defense_detection_v4 as v; print(v.DATASET_LAYOUT['dcfm17'])"` reports what actually resolves,
...
- Operating notes: {refml("RUNNING_ML.md")} — the seed-collision check and the `Stacking`
  warning added here
- Data staged: {refml(DML + "/dataset_final/dcfm/Pilot2k", "dataset_final/dcfm/Pilot2k/")}
  (`static/`, `mobile/`) — the [Step 42](#step-42) DCFM campaign, copied from the collaborator's
  `Pilot2k/` bundle; 2,002 runs static, 2,002 mobile. First staged as
  `dataset_final/dcfm_17/{{static,mobile}}_canonical/` and moved here on merge to match the
  `CAMPAIGN`-keyed layout already on `main`, which reads the batch folders under their delivered
  names. Note the sibling `dataset_final/dcfm/{{static,mobile}}_{{canonical,mixed}}/` is a
  **different, older** campaign — 10,000 runs, seeded 8.0e8/1.1e9, dated 2026-06-11, on the
  33/95-feature schema — and must not be confused with it
- Outputs: {refml(DML + "/results/30_schema33/paper_v4", "paper_v4/")} —
...
| 15 | **Should TRUST be regenerated in its paper-faithful configuration?** ([Step 40](#step-40)) | The realignment leaves the shipped datasets byte-identical (the harness default runs the forward monitor only), so nothing needs refreshing. But §5 + §7 is a *different* defense behaviourally — alert distribution re-floods control messages — and TRUST2 has been the study's quietest defense ([Step 31](#step-31)). Generating it would add a **third point** to the Trust 1.0 / 2.0 calibration curve of [Step 24](#step-24), at much higher efficacy (+55.9% PDR recovery), and is the sharpest available test of the tradeoff thesis. It must be labelled a new condition, not a refresh. **[Step 43](#step-43) supplies the control point:** the forward-monitor-only configuration is now measured on the 17-observable schema at ROC-AUC 0.5585 / 0.6059 and 9.8% / 11.9% recovery, so a paper-faithful run would be a paired comparison rather than a standalone number |
...
> **Superseded by [Step 40](#step-40).** These sub-modules were rewritten and extended
> on branch `trust-defense` of the working `ns-3-dev` tree: all ten of the paper's
> detection rules (including the prose rule of §5.1 that the numbered formulas omit),
> Section 6 (Formulas 13 and 14) carried in a new signed `PROOF_MESSAGE`, and Section 7
> as a real re-broadcast of the offending control messages rather than an in-process
> bus. The breakdown above describes the version in `master`, which is the version that
> generated every dataset in this report.
""")
...
6. **Adnane, A., Bidan, C. & de Sousa Júnior, R. T.** (2013). *Trust-based security for the OLSR routing protocol*. Computer Communications, 36(10–11), 1159–1171. — **Defense 4 (TRUST)** *(confirmed from the `olsr-trust-defense.h` source header)*; the implementation was realigned to it in [Step 40](#step-40), which also reads its **Table 3** (a raster image inside the PDF) and records that Section 8 reports **detection rate only** — no PDR, throughput or delay
...
> **The gap is now larger, not smaller.** [Step 40](#step-40) rebuilt TRUST's `defense/`
> sub-modules on branch `trust-defense` of a separate `ns-3-dev` tree (six files under
> `src/olsr/model/defense/`, plus `PROOF_MESSAGE` in `olsr-header` and the Formula-15
> enforcement points in `olsr-routing-protocol`). **None of it is in
> `manet-olsr-project` master**, so the TRUST links in this index resolve to the
> pre-realignment defense — which is the correct target for reproducing this report's
> datasets, and the wrong one for reproducing [Step 40](#step-40).
